#１．すべてのリクエストの入り口をindex.phpへリダイレクトとしている

これはwebサーバの設定でpublic/.htaccessファイル内にデフォルトで下記のように設定しているため
````public/.htaccess
